...
export function Inspector() {
  ...
  // Directly derive selectedItem during render with useMemo
  const selectedItem = useMemo<DragItem | null>(() => {
  ...
  return (
    <div className="h-1/2 bg-[var(--panel-bg)] border border-[var(--panel-border)] flex flex-col select-none">
      <div className="h-6 px-3 flex items-center bg-[#2d2d2d] border-b border-[#111]">
        <span className="text-[11px] text-[#ccc] font-medium">Effect Controls</span>
      </div>

      <div className="p-3 flex-1 overflow-y-auto text-[#aaa]">
        {selectedItem ? (
          <div className="space-y-4">
            <div className="font-semibold text-white truncate">{selectedItem.name}</div>
            ...
            <div className="pt-4 mt-4 border-t border-[var(--panel-border)] space-y-1">
              ...
              <div className="text-[10px] text-[#888] flex justify-between">
                ...
                <span className="font-mono">
                  {((selectedItem.start || 0) + (selectedItem.duration || 0)).toFixed(2)}s
                </span>
              </div>
            </div>
          </div>
